...
import {
  getAllView,
  searchView,
  type VotosMunicipio2022RJView,
  type PaginationInfo,
} from '../../../../../services/modules/V1/votosMunicipio2022RJService'
import VotosMunicipio2022RJDataTable from './VotosMunicipio2022RJDataTable'
    ...
    rows: [
      {
        fields: [
          {
            col: 12,
            id: 'votos-municipio-2022-search',
            name: 'votos-municipio-2022-search',
            placeholder: 'Buscar por município, cargo, partido, candidato ou situação…',
            value: query,
            onChange,
          },
        ],
...
function VotosMunicipio2022RJList() {
  const navigate = useNavigate()
  const { login: theme } = getActiveTheme()
  const user = getUser()

  const [items, setItems] = useState<VotosMunicipio2022RJView[]>([])
      ...
      <div className="card shadow-lg border-0" style={{ borderRadius: '1rem', overflow: 'hidden' }}>
        ...
        >
          <h1 className="mb-0 fw-semibold h5" style={{ color: theme.headerText, letterSpacing: '0.03em' }}>
            Eleições 2022 — Votos por Município
          </h1>
        </div>
        ...
        <div className="card-body p-3">
          ...
          {!loading && (
            <>
              <VotosMunicipio2022RJDataTable items={items} query={query} />
              {pagination && (
                <Pagination
...
export default VotosMunicipio2022RJList
